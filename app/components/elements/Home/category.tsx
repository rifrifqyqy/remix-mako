        ...
        <ul className="flex w-fit flex-col gap-4">
          {dataCategory.map((category) => (
            <div
              ...
              tabIndex={0}
              className={`menu group/item flex w-full cursor-pointer items-center justify-between gap-44 rounded-full border border-zinc-300 p-2 text-center capitalize transition-all ${activeCategory?.name === category.name ? "bg-main-50 text-white" : "bg-zinc-50"}`}
            >
        ...
        <section className="relative h-[480px]">
          <AnimatePresence>
            <motion.img
              ...
              className="absolute h-fit w-full rounded-xl"
            />
          </AnimatePresence>
        </section>
      </section>
