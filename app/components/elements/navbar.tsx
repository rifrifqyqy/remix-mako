...
const navStyle = ({ isActive, isPending }: { isActive: boolean; isPending: boolean }) => {
  if (isPending) return "pending";
  return isActive ? "text-main-50 font-semibold bg-light-300 px-4 py-2 rounded-full text-[18px]" : "px-4 py-2 rounded-md text-zinc-500 hover:text-main-50 transition-all  text-[18px] font-medium";
};
...
export default function NavbarRemix() {
  ...
  return (
    <>
      <motion.nav variants={AniNav} initial="hidden" animate="visible" className="px-4 my-6 mx-8 border-[2px] rounded-xl border-zinc-300 justify-between py-2 flex items-center sticky top-4 bg-white z-[9999]">
        <NavLink to="/">
          <motion.svg variants={icon} initial="hidden" animate="visible" whileHover="hovering" viewBox="0 0 344 254" fill="none" className="h-10 cursor-pointer" xmlns="http://www.w3.org/2000/svg">
            <motion.path
              variants={icon}
        ...
        <RemixButton color="bg-main-50" textstyle="font-semibold uppercase" style="rounded-full" onClick={handleMenu}>
          Shop Now
        </RemixButton>
      </motion.nav>
    </>
